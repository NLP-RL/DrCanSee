...
class User(object):
    def __init__(self, goal_set, disease_symptom, parameter):
        ...
        self.IP = pickle.load(open(self.parameter.get("image_prediction"), 'rb'))
        self.MS = parameter['MSymp']
        self.MSDict = parameter['MDict']
        self.tag = parameter['image_prediction']
    ...
    def initialize(self, dataset, goal_index=None):
        ...
        goal = self.goal["goal"]
        consult_id = self.goal['consult_id']
        #print('Goal:',goal)
        ...
        for slot in list(goal["explicit_inform_slots"].keys()):
            if slot in inform_slots:
                print('slotttt:',slot)
                if slot in self.MS:
                    self.n = random.randint(0,(len(self.IP[slot])-1))
                    slt = self.IP[slot][self.n]
                    self.state["action"] = "inform"
                    self.state["inform_slots"][slt] = True
                else:
